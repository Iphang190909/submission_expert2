...
class MovieFilmTab extends StatelessWidget {
                  ...
                  return Column(
                    children: [
                      _buildLabel(
                        context,
                        'Now Playing',
                        state.nowPlayingMovieFilm,
                      ),
                      const SizedBox(height: 16),
                      state.nowPlayingMovieFilmViewState.maybeWhen(
                        orElse: () =>
                            const Center(child: Text('Unexpected Error')),
                        loading: () => const CardSkeleton(),
                        success: () =>
                            _buildSuccess(context, state.nowPlayingMovieFilm),
                        error: () =>
                            const CardError(message: 'Something Went Wrong'),
                      ),
                      const SizedBox(height: 16),
                      _buildLabel(context, 'Popular', state.popularMovieFilm),
                      const SizedBox(height: 16),
                      state.popularMovieFilmViewState.maybeWhen(
                        orElse: () =>
                            const Center(child: Text('Unexpected Error')),
                        loading: () => const CardSkeleton(),
                        success: () =>
                            _buildSuccess(context, state.popularMovieFilm),
                        error: () =>
                            const CardError(message: 'Something Went Wrong'),
                      ),
                      const SizedBox(height: 16),
                      ...
                      const SizedBox(height: 16),
                      state.topMovieFilmViewState.maybeWhen(
                        orElse: () =>
                            const Center(child: Text('Unexpected Error')),
                        loading: () => const CardSkeleton(),
                        success: () =>
                            _buildSuccess(context, state.topMovieFilm),
                        error: () =>
                            const CardError(message: 'Something Went Wrong'),
                      )
                    ],
